...
### New count matrix construction for mRNAs

index from cds fasta
```{bash}
/home/shared/kallisto/kallisto \
index -i /home/shared/8TB_HDD_02/zbengt/github/oyster-lnc/output/11-mRNA-counts-DESeq2/kallisto-index/GCF_002022765.2_C_virginica-3.0_cds_from_genomic.index \
...
OUTDIR="/home/shared/8TB_HDD_02/zbengt/github/oyster-lnc/output/11-mRNA-counts-DESeq2"
INDEX="${OUTDIR}/kallisto-index/GCF_002022765.2_C_virginica-3.0_cds_from_genomic.index"

FASTQ_DIR=~/github/oyster-lnc/data/01-lncRNA-kallisto
QUANT_DIR="${OUTDIR}/kallisto-quant"

mkdir -p "${QUANT_DIR}"

for R1 in "${FASTQ_DIR}"/*_R1.fastp-trim.20bp-5prime.20220224.fq.gz
do
  sample=$(basename "${R1}" | sed 's/_R1\.fastp-trim\.20bp-5prime\.20220224\.fq\.gz//')
  R2="${FASTQ_DIR}/${sample}_R2.fastp-trim.20bp-5prime.20220224.fq.gz"
...
tx2gene table from CDS FASTA headers to get gene level (mRNA) matrix
```{r}
library(Biostrings)
library(dplyr)
library(stringr)
...
fasta_file <- "~/github/oyster-lnc/data/11-mRNA-counts-DESeq2/cds_from_genomic.fna"
out_file <- "/home/shared/8TB_HDD_02/zbengt/github/oyster-lnc/output/11-mRNA-counts-DESeq2/tx2gene.tsv"

cds <- readDNAStringSet(fasta_file)
hdr <- names(cds)

tx2gene <- tibble(header = hdr) %>%
  mutate(
...
write_tsv(tx2gene, out_file)

tx2gene %>% head()
```

import kallisto abundances and summarize gene level counts with tximport
...
quant_dir <- "/home/shared/8TB_HDD_02/zbengt/github/oyster-lnc/output/11-mRNA-counts-DESeq2/kallisto-quant"
tx2gene_file <- "/home/shared/8TB_HDD_02/zbengt/github/oyster-lnc/output/11-mRNA-counts-DESeq2/tx2gene.tsv"
out_dir <- "/home/shared/8TB_HDD_02/zbengt/github/oyster-lnc/output/11-mRNA-counts-DESeq2"

samples <- list.dirs(quant_dir, recursive = FALSE, full.names = FALSE)

files <- file.path(quant_dir, samples, "abundance.tsv")
names(files) <- samples

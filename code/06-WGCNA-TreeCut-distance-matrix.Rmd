...
From Ariana's original code here:
...
countsF <- counts[, female_samples$sample]
head(countsF)
```
...
Before filtering, we had 4552 genes. After filtering for pOverA, we have approximately 4190 genes. 
...
```{r}
save(datExpr, file = "~/github/oyster-lnc/data/datExpr.RData")
```
...
Power of 3 from previous use of pickSoftThreshold.
...
We had 12 modules before merging.  
...
We have 9 modules after merging with 85% similarity.   

Plot new tree with 9 modules.  
```{r}
#Calculate dissimilarity of module eigengenes
MEDiss = 1-cor(MEs)
#Cluster again and plot the results
...
We have 11,475 genes and 38 samples.  
...
"ME2"  "ME6"  "ME3"  "ME9"  "ME5"  "ME12" "ME1"  "ME7"  "ME8" 
...
plot_MEs<-Strader_MEs%>%
  gather(., key="Module", value="Mean", ME2:ME8)

write_csv(plot_MEs, "Mcap2020/Output/TagSeq/GenomeV3/WGCNA_eigengene_expression.csv")

plot_MEs<- read_csv("Mcap2020/Output/TagSeq/GenomeV3/WGCNA_eigengene_expression.csv")
#dev.off() 
...
expression_plots<-plot_MEs%>%
  ...
  ggplot(aes(x=lifestage, y=Mean, group=lifestage, colour=group)) +
  facet_wrap(~ Module)+
...
ggsave(expression_plots, file="Mcap2020/Figures/TagSeq/GenomeV3/expression_eigengene.jpeg", height=8, width=10)
```
